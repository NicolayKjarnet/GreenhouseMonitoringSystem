...
#define PushsaferKey "" // Get key here: https://www.pushsafer.com/en/faq#answer1
#define NUMPIXELS 1
#define DATAPIN 33
#define CLOCKPIN 21
...
// Weathermap variable
const char *weathermapEndpoint = ""; // Get url here: https://openweathermap.org/guide

uint8_t lastInternetCheckTime = 0;
uint8_t lightStartTime = 0;
uint8_t heaterSwitchCounter = 0;
 ...
 * @param id The greenhouse id (each greenhouse has its own unique id)
 * @param temp The temperature value
 * @param hum The humidity value
 * @param lux The light value
 */
void sendGreenhouseData(const char *host, int port, int id, float temp, float hum, float lux)
{
  ...
  // String dataToSend = "{\"greenhouseId\":" + String(id) + ",\"temperature\":" + String(temp) + ",\"humidity\":" + String(hum) + ",\"light\":" + String(lux) + "}";
  String dataToSend = "{\"temperature\":" + String(temp) + ",\"humidity\":" + String(hum) + ",\"light\":" + String(lux) + ",\"greenhouseId\":" + String(id) + "}";
  int dataStringLength = dataToSend.length();

  client.print((String) "POST /greenhouses HTTP/1.1\r\n" +
               "Host: " + String(host) + "\r\n" +
 ...
 * @param url endpoint for the openweathermap
 * @param location where the weather info should be read
 */
void postAlert(const char *url, int minsDoorHasBeenOpened)
{
  if (!doorAlertSent)
  {
      ...
      if (httpCode > 0)
      {
        if (httpCode == HTTP_CODE_OK)
        {
          ...
          else
          {
            ...
            input.message = "The door has been open for " + String(minsDoorHasBeenOpened) + " minutes. Current temperature outside is: " + String(temp - 273.15) + "°C" + ". Humidity: " + String(hum) + "%";
 ...
 * and if the door has been open for longer than a set time (2 mins), sends an alert with weather info.
 * If the door is still open after set time (20 mins), sends another alert.
 */
void readMagnet()
{
  ...
  if (magnetNearSensor && doorTimer >= 120000 && !doorAlertSent)
  {
    Serial.println("No magnet near sensor for 2 minutes. Sending alert.");
    postAlert(weathermapEndpoint, doorTimer / 60000); // Converting to minutes

    // Resetting
    doorAlertSent = true;
    doorTimer = 0;
  }
  else if (doorAlertSent && doorTimer >= 1080000)
  {
    Serial.println("No magnet near sensor for 20 minutes. Sending new alert.");
    postAlert(weathermapEndpoint, doorTimer / 60000); // Converting to minutes
  ...
  doorTimer = millis(); // Updating doorTimer to current millis() value
}
...
void loop()
{
  ...
  if ((millis() - lastInternetCheckTime) > timerDelay)
  {
    ...
    sendGreenhouseData(hostDomain, hostPort, 1, temperature, humidity, luxToInt); // greenhouse 1
    // sendGreenhouseData(hostDomain, hostPort, 2, temperature, humidity, luxToInt); // greenhouse 2
    // sendGreenhouseData(hostDomain, hostPort, 3, temperature, humidity, luxToInt); // greenhouse 3
  }
  delay(1000);
}
